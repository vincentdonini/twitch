            ...
            {item.labels.length ? (
              <div className="flex items-center gap-2">
                {item.labels.map((label) => (
                  <Badge key={label} {...getBadgeVariantFromLabel(label)} className="cursor-pointer">
                    {label}
                  </Badge>
                ))}
              </div>
...
function getBadgeVariantFromLabel(label: string): { variant?: "outline"; color?: "default" | "secondary" } {
  if (["work"].includes(label.toLowerCase())) return { color: "default" }
  if (["personal"].includes(label.toLowerCase())) return { variant: "outline" }
