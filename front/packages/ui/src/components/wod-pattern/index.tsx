...
import { formatSecondsToDynamic } from "./helpers"
...
export function WodPattern({ variants }: WodPatternProps) {
  ...
  const handleDivisionChange = (divisionId: string) => {
    ...
    if (!available.includes(selectedGender)) setSelectedGender(available[0] ?? Gender.Mixed)
  }

  if (divisions.length === 0) return null

  return (
    <Tabs defaultValue={divisions[0]!.id} onValueChange={handleDivisionChange}>
      <TabsList variant="line" className="w-full flex justify-start">
        {divisions.map(d => (
          <TabsTrigger key={d.id} value={d.id} className="text-lg">{d.title}</TabsTrigger>
        ))}
      </TabsList>
      ...
      {divisions.map(d => {
        ...
        const activeVariant = divisionVariants.find(v => v.gender === selectedGender) ?? divisionVariants[0]!
            ...
            {(activeVariant.rounds != null || activeVariant.timeCap != null) && (
              <div className="flex flex-wrap items-center gap-3 text-sm font-medium text-muted-foreground">
                ...
                {activeVariant.timeCap != null && (
                  <span>{t("time_cap", { value: formatSecondsToDynamic(activeVariant.timeCap) })}</span>
                )}
              </div>
            )}
